...
import { PIXEL_MAP } from "./pixel-maps";
import type { FC } from "react";

// CUSTOMIZE YOUR TEXT HERE
// const PRIMARY_TEXT = "CYBER PONG";
// const SECONDARY_TEXT = "GAME 2077";
...
// CYBER THEME COLORS
const COLOR = "#00FFFF"; // Neon cyan
const HIT_COLOR = "#FF00FF"; // Neon magenta
const BACKGROUND_COLOR = "#000B1E"; // Dark blue-black
const BALL_COLOR = "#FFFF00"; // Neon yellow
const PADDLE_COLOR = "#00FF00"; // Neon green
...
const PromptingIsAllYouNeed: FC<PromptingProps> = ({
    PRIMARY_TEXT,
    SECONDARY_TEXT,
}) => {
    ...
    useEffect(() => {
        ...
        const initializeGame = () => {
            ...
            pixelsRef.current = [];
            const words = [PRIMARY_TEXT, SECONDARY_TEXT];
            ...
            const calculateWordWidth = (word: string, pixelSize: number) => {
                return (
                    word.split("").reduce((width, letter) => {
                        const letterWidth =
                            PIXEL_MAP[
                                letter.toUpperCase() as keyof typeof PIXEL_MAP
                            ]?.[0]?.length ?? 0;
                        return (
                            width +
                    ...
                    wordIndex === 0
                        ...
                        : words[1].split(" ").reduce((width, w, index) => {
                              return (
                                  ...
                                  (index > 0
                                      ? WORD_SPACING * adjustedSmallPixelSize
                                      : 0)
                              );
                          }, 0);
                ...
                if (wordIndex === 1) {
                    word.split(" ").forEach((subWord) => {
                        subWord.split("").forEach((letter) => {
                            ...
                            startX +=
                                (pixelMap[0].length + LETTER_SPACING) *
                                pixelSize;
                        });
                    ...
                    word.split("").forEach((letter) => {
                        ...
                        startX +=
                            (pixelMap[0].length + LETTER_SPACING) * pixelSize;
                    });
                }
                startY +=
                    wordIndex === 0 ? largeTextHeight + spaceBetweenLines : 0;
            });
        ...
        const updateGame = () => {
            ...
            pixelsRef.current.forEach((pixel) => {
                if (
                    ...
                    if (
                        Math.abs(ball.x - centerX) > Math.abs(ball.y - centerY)
                    ) {
                        ball.dx = -ball.dx;
                    } else {
                        ball.dy = -ball.dy;
        ...
        const drawGame = () => {
            ...
            // Create a gradient background for cyber effect
            const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
            gradient.addColorStop(0, BACKGROUND_COLOR);
            gradient.addColorStop(1, "#001F3F"); // Darker blue at bottom
            ctx.fillStyle = gradient;
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            // Add grid lines for cyber effect
    ...
    return (
        <canvas
            ...
            aria-label="Dynamic Pixel Text Pong Game with Cyber Theme"
        />
    );
};
